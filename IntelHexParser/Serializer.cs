...
namespace Coshx.IntelHexParser {
    public class Serializer {
        ...
        public byte[] Deserialize(String source) {
            ...
            byte[] outcome;
            int recordIndex = 0, finalDataSize = 0;
            ...
            foreach (String l in lines)
            {
                ...
                finalDataSize = Math.Max(current.Address + current.DataLength, finalDataSize); //+= current.DataLength;
            }

            //outcome = Enumerable.Repeat((byte)0xFF, finalDataSize).ToArray();
